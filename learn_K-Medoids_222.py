...
loan_data = pd.read_excel("D:\\数据处理\\源数据\\一周超高峰系数汇总.xlsx",
                          sheet_name="Sheet3", header=0, names=None)

# 设置要进行聚类的字段
loan = np.array(loan_data[['周内早进站', '周内早出站', '周内晚进站', '周内晚出站', '周末早进站', '周末早出站', '周末晚进站', '周末晚出站']])

before = KMedoids(n_clusters=5)  # 降维前
before.fit(loan)
