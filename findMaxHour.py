...
for t in times:
    ...
    # result_data = pd.DataFrame({'车站', '早进站时间', '早进站系数', '早进站1h', '早出站时间', '早出站系数', '早出站1h', '下午进站时间', '下午进站系数', '下午进站1h', '下午出站时间', '下午出站系数', '下午出站1h'})
    result_data = []
    result_data.append(('车站', '早进站时间', '早进站系数', '早进站1h', '早出站时间', '早出站系数', '早出站1h', '下午进站时间', '下午进站系数', '下午进站1h', '下午出站时间', '下午出站系数', '下午出站1h'))
    for i in station:
        print('正在计算20190%s' % t)
        result = []
        result.append(i)
        ...
        result.append(time1[0:5] + '-' + time2[8:])  # 时间
        result.append(x * 4 / y)
        result.append(y)
    ...
    pd.DataFrame(result_data).to_excel('D:/数据处理/dealwith2/20200709全数据20190%s线路小时高峰.xlsx' % t)
print('完成')
